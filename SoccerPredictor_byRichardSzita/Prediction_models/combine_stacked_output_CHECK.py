...
# Set up logging
log_file_path = './SoccerPredictor_byRichardSzita/Prediction_models/test/log/stacked_' + model_type + '_model_training.log'
...
column_order = ['running_id', 'Date', 'league', 'Home', 'Away', 'Prediction_models', 'home_goals_prediction_rounded','away_goals_prediction_rounded'
               , 'outcome_prediction_rounded'
                 ,'home_goals_prediction','away_goals_prediction', 'outcome_prediction']
...
stacked_home_path = './predicted_home_goals_output_CHECK.xlsx'
stacked_away_path = './predicted_away_goals_output_CHECK.xlsx'
stacked_outcome_path = './predicted_goals_output_CHECK.xlsx'
...
stacked_outcome = stacked_outcome[['running_id','outcome_prediction']].sort_values(by='running_id')
...
stacked_merged_df['outcome_prediction_rounded'] = stacked_merged_df['outcome_prediction'].apply(round_half_up).astype(int)
...
# stacked_merged_df['real_score'] = stacked_merged_df['home_goals'].astype(str) + '-' + stacked_merged_df['away_goals'].astype(str)
print(list(stacked_merged_df.columns))
stacked_merged_df = stacked_merged_df[column_order]
# Print the updated DataFrame
stacked_merged_df.to_excel('./predicted_goals_output_CHECK_merged.xlsx')