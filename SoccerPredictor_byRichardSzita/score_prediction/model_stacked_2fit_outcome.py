...
# Set up logging
log_file_path = './SoccerPredictor_byRichardSzita/score_prediction/log/stacked_' + model_type + '_model_training.log'
...
base_data_path = './SoccerPredictor_byRichardSzita/data/model_data_training_newPoisson.xlsx'
new_prediction_path = './SoccerPredictor_byRichardSzita/data/merged_data_prediction.csv'
real_scores_path = './SoccerPredictor_byRichardSzita/made_predictions/predictions_stacked_2fit_merged.xlsx'
...
data = base_data.drop(columns=['Unnamed: 0.1', 'Unnamed: 0','match_outcome', 'home_goals','away_goals',  'draw', 'away_win', 'home_win','away_points', 'home_points','HomeTeam_last_away_match','AwayTeam_last_home_match','home_points_rolling_avg','away_points_rolling_avg','home_advantage'], errors='ignore')
...
def make_prediction(model_type, model, prediction_data, residual_model):
    ...
    output_file = f'./SoccerPredictor_byRichardSzita/predictions_hybrid_2fit_{model_type}.xlsx'
...
try:
    if len(data_with_error)>0:
        ...
        logging.info("Train a residual model for error")
        ...
        y_train_home_error = data_with_error['outcome_error']
